...
find_logOR <- function(my.dips, before.sel){
  a <- my.dips                        %>% 
    mutate(z = before.sel )           %>%
    group_by(F.mat)                   %>%
    summarise(p_a = sum((1-A.pat)*z), 
              p_A = sum(A.pat*z)    ,
              odds = p_a/p_A)
  return(log(a$odds[1] / a$odds[2] ))
}
...
runThreLocusSim <-function(n.gen = 1000, r12 = .1, r23 = .3,
  ...
  colnames(geno.time) <- c("gen", apply(unique(diplos[,1:7]), 1, paste, collapse = ""),"reinfpop0","reinfpop1")
  for(g in 1:n.gen){
    ...
    pop1 <- migrateMateReproduceSelect(diplos = diplos, 
                                       ...
                                       discrim = discrim, 
                                       s = s)
    diplos$freqs  <- c(pop0$after.sel, pop1$after.sel)
    geno.time[g,] <- c(g,diplos$freqs , pop0$logOR, pop1$logOR)
  }
...
#AMF   
full_output        <- runThreLocusSim(n.gen = 1000, discrim = 1, s=.1, r12=.01, r23 = 0.0001) 

stalled_definition_attempt <-  full_output %>% 
  select(reinfpop0, reinfpop1)
...
output_freq %>% 
  ...
  gather(key = locus, value = freq, - pop, -gen) %>%
  filter(locus == "f") %>%
  ggplot(aes(x=gen,y=1-freq,color=locus )) + 
  geom_line() + 
  facet_wrap(~pop, labeller = "label_both")+
  scale_x_continuous(trans = "log10") +
  ggtitle("F frequency over time")+
  theme_tufte()
